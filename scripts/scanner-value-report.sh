...
#
# Reads run_score events from $DEVKIT_PROJECT_DIR/plans/audit-logs/ship-*.jsonl ONLY.
...
#   --audit-log-dir P   Override audit log directory
...
Reads:
  run_score events from $DEVKIT_PROJECT_DIR/plans/audit-logs/ship-*.jsonl ONLY
...
while [[ $# -gt 0 ]]; do
    case "${1:-}" in
        --format)
            ...
            ;;
        --audit-log-dir)
            AUDIT_LOG_DIR="${2:-${DEVKIT_PROJECT_DIR:-.devkit}/plans/audit-logs}"
            shift 2
            ;;
        --help|-h)
